...
void ReServer::setHandlers()
{
    ...
    on("/panel/msg", HTTP_GET | HTTP_POST, (ArRequestHandlerFunction)std::bind(&ReServer::panelMessageHandler, this, std::placeholders::_1));
}
...
void ReServer::panelMessageHandler(AsyncWebServerRequest *request)
{
    String msg;

    if (request->hasParam("msg") && !request->getParam("msg")->value().isEmpty())
    {
